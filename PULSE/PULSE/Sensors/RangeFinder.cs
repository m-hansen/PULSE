...
namespace PulseGame
{
    public class RangeFinder : Sensor
    {
        ...
        public bool IsIntersecting;             // if the rangefinder is currently intersecting with an object

        private Vector2 intersectingPoint;      // stores the point of intersection between the sensor and object
        private Vector2 IntersectionNormal;
        ...
        public override void Update(List<GameAgent> agentAIList, Vector2 playerPos, float playerRot)
        {
            ...
            endPoint = Utils.CalculateRotatedMovement(new Vector2(0, -1), playerRot + Rotation) * MaxDistance + playerPos;
            ...
            foreach (GameAgent agent in walls)
            {
                ...
                CheckIntersection(agent, isVerticalLine, slope, offset, playerPos, endPoint);
            }
        }
        ...
        private bool IsLineIntersection(Vector2 targetStartPoint, Vector2 targetEndPoint, bool isVerticalLine,
                                        float slope, float offset, Vector2 rfStartPoint, Vector2 rfEndPoint,
                                        Vector2 wallNormal)
        {
            ...
            if (isVerticalLine)
            {
                ...
                {
                    CalculateDistanceToIntersection(rfStartPoint, new Vector2(rfStartPoint.X, targetStartPoint.Y), wallNormal);
                    return true;
                }
            ...
            if (targetStartPoint.X == targetEndPoint.X)
            {
                ...
                {
                    CalculateDistanceToIntersection(rfStartPoint, new Vector2(targetStartPoint.X, intersectingYPos), wallNormal);
                    return true;
                }
            ...
            {
                CalculateDistanceToIntersection(rfStartPoint, new Vector2(intersectingXPos, targetStartPoint.Y), wallNormal);
                return true;
            }
        ...
        private void CalculateDistanceToIntersection(Vector2 startPos, Vector2 endPos, Vector2 wallNormal)
        {
            // calculating the distance between the two points
            double distance = Vector2.Distance(endPos, startPos);//Math.Sqrt(Math.Pow(endPos.Y - startPos.Y, 2) + Math.Pow(endPos.X - startPos.X, 2));
            ...
            if (intersectionDistance == 0.0f || distance < intersectionDistance)
            {
                ...
                IntersectionNormal = wallNormal;
            }
        }

        public override void Draw(SpriteBatch sprites, Vector2 startPoint, Rectangle visibleRect, SpriteFont font1)
        {
            if (Active)
            {
                ...
                (
                    startPoint - topLeftVisibleSpot,
                    (IsIntersecting ? intersectingPoint : endPoint) - topLeftVisibleSpot, 
                    IsIntersecting ? Color.Red : Color.Yellow
                );
        ...
        private float GetRotationForSteeringForce(Vector2 steeringForce)
        {
        ...
        public Vector2? AdjustRotationForCollision(Vector2 currentPos)
        {
            if (!IsIntersecting) return null;

            Vector2 overShoot = endPoint - intersectingPoint;

            Vector2 steeringForce = IntersectionNormal * overShoot.Length();

            return steeringForce / 3;
        }
    }
}
